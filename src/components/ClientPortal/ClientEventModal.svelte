<script>
	...
	let date_created = new Date(open_modal_object.created.seconds * 1000);
...
<div id="event-modal" in:fly={{ delay: 0, duration: 400, x: 0, y: 100, opacity: 0 }}>
	<button
		id="close-modal"
		on:click={() => {
			dispatchEvent('modal_close_click', open_modal_object.eid);
		}}
	>
	...
	<div id="modal-body">
		<div id="modal-info" class="column">
			<div id="event-type" class="info-row">
				<div id="type-label" class="label">Event Label:</div>
			...
			<div id="event-status" class="info-row">
				<div id="status-label" class="label">Event Category:</div>
				<div id="status-variable" class="variable">{open_modal_object.category}</div>
			</div>

			<!-- loop through any other custom attributes -->
			{#each open_modal_object.custom_data as attribute}
				<div class="info-row">
	...
	<div id="modal-footer">
		...
		<div id="timestamp" class="footer-row">
			...
			<div id="timestamp-full">{date_created}</div>
		</div>
	</div>
</div>
